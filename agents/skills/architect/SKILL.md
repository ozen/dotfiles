...
Create a visible checklist with one entry per phase before starting. Use the environment's planning mechanism when available; otherwise maintain it in the working notes. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use Arena's runner count. Every runner inherits the main agent's model because Arena omits the `model` field.
