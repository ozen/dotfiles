...
Create a visible checklist whose first item is to read the Principles section of the **poteto-mode** skill. Use the environment's planning mechanism when available; otherwise maintain it in the working notes. Then add the phases below as checklist items.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with independent, isolated, opinionated candidates and a fresh read-only judge. Every spawn inherits the main agent's model. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
...
Then put the design into motion. Add its steps to the checklist as concrete items, after the Phase C entry and before Phase D. Run each under the Phase C iteration discipline, and weave the Phase D log through them, a row as each step lands, rather than saving the whole trail for the end.
