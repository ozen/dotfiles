...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use the environment's file-reading, text-search, and file-listing capabilities as needed. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch.
