---
name: no-comments
description: "Run an independent comment-focused review, fix accepted findings, and offer encodings for claimed constraints."
disable-model-invocation: true
---

# No comments

Run an independent comment-focused reviewer. Act on accepted findings.

Authoring agents defend comments. Defer to a fresh reviewer's perspective.
...
1. Delegate the scope to a fresh comment-focused reviewer through the environment's available agent launcher. Omit any model override so it inherits the main agent's model. Ask it to inspect only comments and suppressions, remove comments that merely narrate code or preserve changeable implementation constraints, retain comments that explain externally imposed constraints, and label questionable removals as `MUST KILL` with a reason. It must not make unrelated application-code edits. If delegation is unavailable, run that same review in the current session as a separate pass and keep its findings distinct from implementation work.
