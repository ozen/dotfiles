...
**Start every multi-step task with a visible checklist whose first item is to read the Principles section below in full.** Use the environment's planning mechanism when available; otherwise maintain it in the working notes. The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the environment's available skill-authoring workflow.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → the available `deslop` skill (`/deslop`).
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → drive the matching user-facing surface through whatever browser, UI, terminal, CLI, native-app, simulator automation, or repository harness the environment provides. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception. If no driver or harness exists, live verification is `BLOCKED` or `INCONCLUSIVE`, never `PASS`.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`), not a competing generic babysit workflow whose description matches the same words. That includes "babysit this", "get it green", "address the bugbot comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling; the playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
...
**Just do it.** Use any available connected integration. Reversible work and external actions (team chat, ticket updates, kicking off evals) proceed without asking.
...
Read `references/runtime-capabilities.md` before a playbook delegates work, requests user input, persists an objective, schedules a wakeup, selects execution placement, or drives a user-facing surface.

Use the environment's available agent launcher for every playbook worker or ad-hoc helper. Launch independent work concurrently when supported, give each worker isolated writable state, pass file pointers instead of inlined context, and omit model overrides. Every subagent inherits the main agent's model. Never select, configure, escalate, or vary models by role, difficulty, reviewer, worker, or retry. If delegation or concurrency is unavailable, run the same briefs serially with separate outputs; do not omit a role or reduce the requested count.

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Interrupt-chained resumes silently drop directives, so fire a fresh subagent with consolidated scope rather than trusting a "done" summary. A second opinion is the same prompt in a fresh subagent context. Agreement is high-signal.
...
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a transcript, remote-agent URL, or pushed branch. `playbooks/session-pickup.md`.
