...
- The judge can know it's judging but sees outputs by sanitized label only, never by candidate identity.
...
4. **Spawn N parallel candidates** per the **arena** skill's Phase B. Each works in its own sanitized dir with the same prompt. Omit `model` so every candidate inherits the main agent's model.
5. **Spawn one blinded judge** per the **arena** skill's Phase C. Omit `model` so the judge inherits the main agent's model. The judge sees outputs by sanitized label and the rubric, never candidate identity.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript under the active workspace's `agent-transcripts/` directory named by the system prompt or environment metadata. Do not search transcript roots belonging to other workspaces; that crosses workspace boundaries and reads private chats from unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
7. **Read every candidate output yourself** end to end. Compare to the judge's verdict. Disagreement means the judge or parent may be biased, or the rubric is ambiguous. Synthesize.
