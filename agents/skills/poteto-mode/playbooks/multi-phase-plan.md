...
3. Explore with delegated agents through the environment's available launcher. Omit model overrides so each inherits the main agent's model (the **guard-the-context-window** principle skill). Launch independent angles concurrently when supported; otherwise run the same briefs serially and keep their outputs separate. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
**Verification.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked (the **prove-it-works** principle skill). That sentence is the verification rule. Every verification block opens with it. The live block is mandatory. Ten lanes at the PR head inherit the main agent's model and drive the real surface through available surface automation or a repository harness, per the **swarm** skill. Each lane is one box with a concrete scenario, the screenshot it saves, and its pass predicate. One lane is the **Regression lane against trunk.** It runs the same load-bearing scenario on trunk and head. If trunk does not have the feature, the lane records that fact and gates the behavior the diff adds plus the end state the user waits for instead of inventing a trunk result. The perf gate is dual-sided: trunk and head must both produce the named metric. If trunk lacks the feature, also isolate the work the diff adds and set an absolute budget for that work plus the end-to-end state the user waits for; do not claim a ratio between unlike scenarios. The perf block names the metric, the interleaved probe, the trunk baseline measured first, and the rule with the number that fails. A PR that changes an interaction is review-gated. The operator reviews it in chat with screenshots and a video before merge. A PR that changes no interaction writes `**Review gate.** None. <PR id> is not review-gated.` and no boxes under it.

**Surface driver.** Pick it by surface. Browser, Electron, and web UIs use available browser or UI automation. CLIs and TUIs use terminal or CLI driving. Native mobile uses available simulator automation. A repository harness is the fallback only when it exercises the same user-facing surface. A PR that touches two surfaces gets lanes on both. A surface with no driver or harness is a risk in Appendix C, and its live block records `INCONCLUSIVE` rather than inventing a pass.
...
- [ ] On her go, persist a durable objective with this exact text. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>" Use the environment's native durable-objective mechanism when available; otherwise store it with the program state together with the current state, next check, cadence, stop conditions, and authorization boundaries.
- [ ] Read these from trunk at program start. Re-read them at every tick.
  ...
  - [ ] `git show origin/main:<surface-driver or repository-harness instructions>`
...
- [ ] Arm the 30-minute audit tick through the environment's event waiter or recurring scheduler. Use a monitored-shell interval only when the runtime can observe and resume it. If no wake mechanism can outlive the current run, persist a resumable checkpoint and report that automatic continuation is unavailable. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook from trunk and the durable objective. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then send the operator a status message, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
...
Each live lane runs on its own isolated environment at the PR head. Drive it through available browser, UI, terminal, CLI, native-app, or simulator automation, or through a repository harness that exercises the same surface. If neither is available, return `INCONCLUSIVE`.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes inheriting the main agent's model at the PR head, per the boot recipe.
