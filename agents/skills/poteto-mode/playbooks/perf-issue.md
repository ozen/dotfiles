...
1. Capture a baseline trace through available surface automation, diagnostics, or a repository harness. If no capability can reach the surface, report the investigation `BLOCKED` rather than substituting static inspection.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent and omit `model` so it inherits the main agent's model; review the diff. Capture a post-fix trace.
