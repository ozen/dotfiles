...
5. Verify on the matching surface. For a visual decision, screenshot each variant through available surface automation or a repository harness and drive the interaction; the eye is the test. For a behavioral or timing decision, observe the thing you are deciding by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion. If no driver or harness can reach the surface, mark the result `INCONCLUSIVE` rather than selecting a variant from tests alone.
