...
1. Capture the live signal on the matching surface through available surface automation, diagnostics, or a repository harness: a CPU profile for a spinning process, a heap snapshot for a leak, a CDP trace for a visual glitch. A real artifact, not a guess. If no capability can reach the surface, report the investigation `BLOCKED` rather than substituting static inspection.
