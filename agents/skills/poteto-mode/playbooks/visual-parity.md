...
4. Verify each component against its baseline via image diff on the matching surface using available surface automation or a repository harness. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Iterate per component until the diff is zero. If no surface driver or harness exists, mark verification `INCONCLUSIVE`; tests alone are not a pass.
