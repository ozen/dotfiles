...
Flag every moment the user manually supplied context the agent could have fetched itself via a connected integration (ticket tracker, chat, docs, observability, error tracker, source control, analytics warehouse, CI, design tool, etc.) or another skill.
...
- Routing: the skill that owns the workflow this came up in. Extend it to call the relevant connected integration or sibling skill so the next agent fetches the context itself.

Examples of the pattern:
- User pastes a ticket title because the agent did not query the ticket-tracker integration. Routing: the relevant triage skill should call that integration first.
- User describes a flaky test the agent could have queried via an observability integration. Routing: the debugging skill should mention that integration.
- User links a chat thread the agent could have fetched via a chat integration. Routing: the relevant skill should mention that integration.
...
Findings must point to skills, tools, or connected integrations invoked in this transcript. Speculative routings to skills the parent never opened do not count. To check whether a skill was used, scan the transcript for:

- file reads against any `SKILL.md` file (workspace `.agents/skills/`, user-level `~/.agents/skills/`, or environment-provided plugin directories)
- delegated-agent prompts that name a skill path
- shell, text-search, or connected-integration calls that match a skill's documented commands
