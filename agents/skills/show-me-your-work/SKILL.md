...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the active workspace's `agent-transcripts/` directory named by the system prompt or environment metadata. Do not search transcript roots belonging to other workspaces; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a fresh subagent and omit `model` so it inherits the main agent's model. Self-review is not a substitute; the independent context provides fresh eyes. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with `reviewed by an independent subagent`, then list each flag pointing to specific rows or moments. "No flags" is a valid value. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
