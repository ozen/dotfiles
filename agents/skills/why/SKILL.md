---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', design rationale, regressions, postmortems, or data-backed thresholds. Discovers available connected integrations and queries each evidence category (source control, issue tracker, long-form docs, real-time chat, infrastructure observability, error tracking, product analytics warehouse) in parallel, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, current editor selection, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the integrations and tools available in the current agent environment. Use the available-tools map when present. Otherwise inspect the environment's exposed integration or plugin registry.

Map each available connected integration to one evidence category:
...
Launch all matching investigators concurrently through the available agent launcher. One investigator per category lets each specialize in one integration's query vocabulary and result shape. Do not ask one agent to cover multiple integrations. Omit any model override so every investigator inherits the main agent's model. Retain access to connected integrations while forbidding writes in each brief; use launcher-level write restrictions only when they do not remove that access. If delegation or concurrency is unavailable, run the same investigator briefs serially in the current session and keep their results separate; do not skip an available evidence category.

Each investigator gets:
1. The base prompt from `references/investigator-prompt.md`
2. The category playbook `references/sources/<source>.md` for the selected integration, adapted from the examples in `references/source-playbook.md`
...
Spawn one investigator per category that has a matching integration. Each owns exactly one integration or source-control tool.
...
Launch one fresh synthesizer subagent through the available agent launcher. Omit any model override so it inherits the main agent's model. Its quality check spot-verifies citations, so retain access to connected integrations while forbidding file writes in the brief. If delegation is unavailable, synthesize in the current session and disclose that fresh-context validation was unavailable.
...
**Sources Consulted**. One line per investigator, including the ones that returned nothing. The reader should see at a glance (a) which connected integrations were queried, (b) which came back empty, and (c) which were skipped and why. This coverage map lets the user judge breadth and redirect if something obvious was missed.
...
- **Collapsing investigators into one agent**. Each integration has its own query vocabulary, result shape, and pitfalls; pooling them dilutes specialization and makes coverage harder to reason about. Always one investigator per category.
...
- `references/sources/*.md`. One self-contained example playbook per category, plus cross-cutting `incident-postmortem.md`. Give an investigator the single file that matches its category and adapt it to the available integration.
