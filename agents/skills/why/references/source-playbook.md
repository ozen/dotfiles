...
The why skill spawns one investigator per available evidence category, each reading a single source-specific playbook below. The playbooks are concrete examples for common connected integrations; adapt them for a different integration in the same category.
