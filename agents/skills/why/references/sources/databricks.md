...
- **Databricks notebooks.** Exploratory analyses engineers wrote before code changes. **Not queryable via the SQL integration.** If you suspect the rationale lives in a notebook, name it as a gap.

## How to search it

Use the Databricks SQL integration. Primary tool: `execute_sql_read_only`. If it returns a `statement_id`, poll with `poll_sql_result` rather than re-running.
...
- **Notebooks aren't queryable.** The SQL integration can't see Databricks notebooks. If you suspect the rationale lives in one, return a gap.
