...
Slack integration tools vary. Check which Slack integration is available and inspect its tool schema first. It may require authentication. If authentication fails, stop and report the gap.
...
- **Auth failures.** If the integration isn't authenticated, stop. Don't make up findings. Report that Slack wasn't searchable.
